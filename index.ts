...
const IG_SHARE_FAIL: string = 'ig_share_failure';
 ...
 * @readonly {Promise<void>} A promise with the action to open instagram.
 */
const shareVideoOnInstagram = (videoUrl): Promise<void> => {
  if (Platform.OS === 'ios') {
    return RNShareStory.shareVideoOnInstagram(videoUrl);
  } else {
    Promise.reject('Feature not currently implemented for Android.')
